...
def main():
    if len(sys.argv) < 2:
        print("Uso: python matrizinteractivagamagrid.py <nombre_del_archivo.csv>")
        sys.exit(1)
    ...
    print("Renderizando grilla cuadrada... (Esto puede tomar un momento)")
    for r in range(rows):
        for c in range(cols):
            x_pos = c * width
            y_pos = r * height
    ...
    # --- 4. NUEVO: CAPA DE ISOLÍNEAS (CONTOUR) ---
    contour_layer = [None] 
    
    def toggle_contours(event):
        if contour_layer[0] is None:
            x_grid = np.arange(cols)
            y_grid = np.arange(rows)
            X, Y = np.meshgrid(x_grid, y_grid)
            
            contour_layer[0] = ax.contour(X, Y, grid_data, levels=15, colors='white', alpha=0.7, linewidths=1.5)
            ax.clabel(contour_layer[0], inline=False, fontsize=10) #Isolines labels
            ax.set_title("Evaluación de Isotropía: Curvas de Nivel Activas", pad=20, weight='bold')
        else:
            for c in contour_layer[0].collections:
                c.remove()
            contour_layer[0] = None
            ax.set_title(default_title, pad=20)
            
        fig.canvas.draw_idle()

    # Espacio para el botón y creación del mismo
    plt.subplots_adjust(bottom=0.15)
    ax_button = plt.axes([0.4, 0.02, 0.2, 0.05])
    btn = Button(ax_button, 'Ver Isotropía (Isolíneas)')
    btn.on_clicked(toggle_contours)


    # --- 5. EVENTOS INTERACTIVOS ---

    # Formateador inferior (Status bar)
    ...
    # Click: Máximo en vecindad de Moore (3x3)
    highlight_patches = [] 
    ...
    def on_click(event):
        ...
        if 0 <= r_approx < rows and 0 <= c_approx < cols:
            ...
            # Búsqueda en los 8 vecinos y el centro (3x3)
            ...
            # Resalte de la celda clicleada (Café)
            ...
            # Resalte del valor máximo de la vecindad (Rojo)
